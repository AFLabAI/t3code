...
const recoveredMarkdownOffsetDeltaByNode = new WeakMap<object, number>();

function retainRecoveredMarkdownSource(
  node: MarkdownAstNode,
  source: string,
  offsetDelta: number | undefined,
): void {
  recoveredMarkdownSourceByNode.set(node, source);
  if (offsetDelta !== undefined) {
    recoveredMarkdownOffsetDeltaByNode.set(node, offsetDelta);
  }
  for (const child of node.children ?? []) {
    retainRecoveredMarkdownSource(child, source, offsetDelta);
  }
}
...
export function sourceOffsetForRecoveredMarkdownNode(node: object, offset: number): number {
  return offset + (recoveredMarkdownOffsetDeltaByNode.get(node) ?? 0);
}
...
function blocksFromIndentedCode(
  ...
  const valueStart = nodeStart === undefined ? -1 : source.indexOf(value, nodeStart);
  const offsetDelta =
    valueStart < 0
      ? undefined
      : sourceOffsetForRecoveredMarkdownNode(node, valueStart) - INLINE_PARSE_PREFIX.length;
  for (const block of result.blocks) {
    retainRecoveredMarkdownSource(block, result.source, offsetDelta);
  }
  return result;
}
