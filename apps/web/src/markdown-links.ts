...
const SPECIAL_RELATIVE_PATH_SEGMENT_PATTERN = /^[A-Za-z0-9._()[\]-]+$/;
...
export function normalizeMarkdownLinkDestination(value: string): string {
  const unwrappedValue = unwrapMarkdownLinkDestination(value.trim());
  if (WINDOWS_BACKSLASH_PATH_PATTERN.test(unwrappedValue)) {
    return unwrappedValue;
  }
  return unwrappedValue.replace(MARKDOWN_ESCAPE_PATTERN, "$1");
}
...
function isLikelyPathCandidate(path: string): boolean {
  ...
  if (RELATIVE_FILE_PATH_PATTERN.test(path) || RELATIVE_FILE_NAME_PATTERN.test(path)) return true;
  ...
  const segments = actualPath.replaceAll("\\", "/").split("/");
  if (segments.length <= 1 || segments.some((segment) => segment.length === 0)) return false;

  const basename = segments.at(-1) ?? "";
  return (
    /\.[A-Za-z0-9_-]+$/.test(basename) &&
    segments.every((segment) => SPECIAL_RELATIVE_PATH_SEGMENT_PATTERN.test(segment))
  );
}
