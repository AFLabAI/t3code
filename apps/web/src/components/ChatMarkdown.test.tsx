...
describe("ChatMarkdown file option chips", () => {
  ...
  it.each([
    "[Route](apps/web/src/routes/(chat)/[threadId].tsx)",
    "[Route](apps/web/src/routes/%28chat%29/%5BthreadId%5D.tsx)",
  ])("renders special-character file links as file chips", (text) => {
    const html = renderToStaticMarkup(
      <ChatMarkdown
        cwd="/repo/project"
        environmentId={EnvironmentId.make("environment-special-path")}
        text={text}
      />,
    );

    expect(html).toContain('href="/repo/project/apps/web/src/routes/(chat)/[threadId].tsx"');
