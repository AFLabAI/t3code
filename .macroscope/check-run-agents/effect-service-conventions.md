...
- Admit cwd catalogs only after the machine snapshot is authoritative, and single-flight probes by provider-instance generation plus cwd. A pending machine snapshot must remain a fallback, and overlapping turns must not spawn duplicate provider processes.
