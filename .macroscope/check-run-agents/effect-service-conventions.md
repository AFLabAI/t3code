...
- Keep cwd catalog refreshes narrow: query only skills and slash commands needed by the menu. Do not reuse full provider health/model inventory probes for a post-session catalog refresh, because unrelated model/auth commands can block the catalog indefinitely.
- Apply the same bounded-output policy to every CLI path that reads a catalog. A narrow cwd probe must not bypass the output cap used by the machine inventory probe.
...
- Admit cwd catalogs only after both the registry machine snapshot and the scoped probe's base snapshot are authoritative, and single-flight probes by provider-instance generation plus cwd. A pending snapshot from either source must remain a fallback, and overlapping turns must not spawn duplicate provider processes.
