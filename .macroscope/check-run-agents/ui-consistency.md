...
- Keep async loading and empty-state copy aligned with the active trigger and data source. When a shared menu adds a trigger to a pending condition, trace its loading-copy branch so skill discovery cannot fall through to file-search text.
