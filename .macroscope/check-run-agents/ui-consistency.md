...
- When a provider skill catalog becomes workspace-scoped, trace every renderer that resolves `$skill` tokens, including existing-thread composers, new-task drafts, and sent-message timelines on web and mobile. Drafts must use the cwd where their selected mode will actually execute: an explicit worktree path only for local/current-checkout mode, otherwise the project root until a new worktree exists. The same token must not render as a chip on one surface and raw text on another.
